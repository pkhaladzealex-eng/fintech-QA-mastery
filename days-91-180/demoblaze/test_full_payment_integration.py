...
def test_full_payment_integration(browser):
    ...
    charge = utils.create_stripe_charge(
        ui_price, product_name, card_token="tok_visa"
    )
    assert charge.status == "succeeded"

    # 4. Insert into Database via Helper
    utils.insert_payment_to_db(
        charge.id, charge.amount, charge.status, charge.description
    )

    # 5. DB Verification and cleanup
    conn = sqlite3.connect(cfg.DATABASE_PATH)
    cursor = conn.cursor()

    try:
        cursor.execute(
            "SELECT amount, description FROM payments WHERE payment_id = ?",
            (charge.id,),
        )
        db_row = cursor.fetchone()

        assert db_row is not None, "Payment record was not found in the database"
